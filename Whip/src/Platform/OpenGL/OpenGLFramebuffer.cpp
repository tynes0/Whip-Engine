...
namespace Utils
{
	...
	static GLenum WhipFramebufferTextureFormatToGL(FramebufferTextureFormat format)
	{
		switch (format)
		{
...
{
	for (auto attachmentSpec : m_Spec.m_Attachments.m_Attachments)
	{
		if (!Utils::IsDepthFormat(attachmentSpec.m_TextureFormat))
...
void OpenGLFramebuffer::Invalidate()
{
	...
	bool multisample = m_Spec.m_Samples > 1;
	...
	if (m_ColorAttachmentSpecifications.size())
	{
		...
		for (size_t i = 0; i < m_ColorAttachments.size(); i++)
		{
			...
			case FramebufferTextureFormat::Rgba8:
				Utils::AttachColorTexture(m_ColorAttachments[i], m_Spec.m_Samples, GL_RGBA8, GL_RGBA, m_Spec.m_Width, m_Spec.m_Height, static_cast<int>(i));
				break;
			case FramebufferTextureFormat::RedInteger:
				Utils::AttachColorTexture(m_ColorAttachments[i], m_Spec.m_Samples, GL_R32I, GL_RED_INTEGER, m_Spec.m_Width, m_Spec.m_Height, static_cast<int>(i));
				break;
			}
		}
	}
	...
	if (m_DepthAttachmentSpecification.m_TextureFormat != FramebufferTextureFormat::None)
	{
		...
		case FramebufferTextureFormat::Depth24Stencil8:
			Utils::AttachDepthTexture(m_DepthAttachment, m_Spec.m_Samples, GL_DEPTH24_STENCIL8, GL_DEPTH_STENCIL_ATTACHMENT, m_Spec.m_Width, m_Spec.m_Height);
			break;
		}
	}
	...
	{
		WHP_CORE_ASSERT(m_ColorAttachments.size() <= 4, "");
...
int OpenGLFramebuffer::ReadPixel(uint32_t attachmentIndex, int x, int y)
{
	...
	WHP_CORE_ASSERT(attachmentIndex < m_ColorAttachments.size(), "attachmentIndex is out of the range!");
...
void OpenGLFramebuffer::ClearAttachment(uint32_t attachmentIndex, int value)
{
	...
	glClearTexImage(m_ColorAttachments[attachmentIndex], 0,
		Utils::WhipFramebufferTextureFormatToGL(spec.m_TextureFormat), GL_INT, &value);
}
