...
class texture_atlas_parser {
public:
	// Constructor: Initialize with a spritesheet
	texture_atlas_parser(const ref<texture2D>& spritesheet)
		: m_spritesheet(spritesheet) {
	}

	// Detect sprites automatically based on non-empty areas
	void detect_sprites_auto() {
		if (!m_spritesheet) {
		...
		for (uint32_t y = 0; y < sheet_height; ++y) {
			for (uint32_t x = 0; x < sheet_width; ++x) {
				...
				if (pixel[3] > alpha_threshold) { // Non-transparent pixel detected
					...
					extract_sprite_safe(start_coords, sprite_size);
				}
			}
		}
		data.release();
	...
	const std::vector<ref<texture2D>>& get_sprites() const {
		return m_sprites;
	}
...
private:
	...
	void extract_sprite_safe(const glm::vec2& coords, const glm::vec2& sprite_size) {
	...
	glm::vec2 find_non_empty_area(const glm::vec2& start_coords, uint8_t* pixels, uint32_t sheet_width, uint32_t sheet_height, uint8_t alpha_threshold, std::vector<std::vector<bool>>& visited) {
		...
		while (!queue.empty()) {
			...
			WHP_CORE_INFO("Updating bounds: min({}, {}), max({}, {})", min_coords.x, min_coords.y, max_coords.x, max_coords.y);
