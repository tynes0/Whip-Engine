...
bool asset_registry::add(asset_handle handle, const asset_metadata& metadata)
{
	...
	if (path_exist(metadata.type, metadata.filepath))
		return false;
	m_registries[*frenum::index(metadata.type)][handle] = metadata;
	return true;
}
...
bool asset_registry::remove(asset_handle handle)
{
	...
	if (!it.second)
		return false;
	m_registries[*frenum::index(it.first->second.type)].erase(it.first);
	return true;
}
		...
		foreach([&out](const value_type& value) 
			{
				...
				out << YAML::Key << "type" << YAML::Value << frenum::to_string(value.second.type);
				out << YAML::EndMap;
			});
		out << YAML::EndSeq;
...
bool asset_registry::deserialize()
{
	...
	for (const auto& node : root_node)
	{
		...
		metadata.type = frenum::cast<asset_type>(node["type"].as<std::string>()).value();
...
{
	const auto& filtered_ar = m_registries[*frenum::index(type)];
	for (const auto& data : filtered_ar)
		if (data.second.filepath == filepath)
			return true;
	return false;
