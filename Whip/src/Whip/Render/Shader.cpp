...
ref<shader> shader::create(const std::string& name, const std::string& vertexSrc, const std::string& fragmentSrc, short test)
{
	switch (renderer::get_API())
	{
		case render_API::API::none:		WHP_CORE_ASSERT(false, "RandererAPI is none!"); return nullptr;
		case render_API::API::opengl:	return make_ref<opengl_shader>(name, vertexSrc, fragmentSrc, test);
	}
...
ref<shader> shader_library::load(const std::string& name, const std::string& vertex_src, const std::string& fragment_src, short test)
{
	auto shad = shader::create(name, vertex_src, fragment_src, test);
	add(name, shad);
	return shad;
}
