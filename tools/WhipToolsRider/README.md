...
- direct Rider Mono debugger attach configuration
- clipboard copy for the `host:port` endpoint
- best-effort opening of Rider's existing attach UI if direct attach cannot be started
...
The plugin starts a dedicated Rider Mono remote attach configuration directly. It intentionally does not
open a raw socket probe before attaching, because Mono's debugger agent expects the first connection to
be the real debugger protocol session.
